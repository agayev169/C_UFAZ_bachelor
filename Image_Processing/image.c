...
void I_saturation (Color *img, int nb_pixels, float coef) {
	for (int i = 0; i < nb_pixels; i++) img[i] = C_saturation(img[i], coef);
}

double I_getSaturation (Color *img, int nb_pixels) {
	double res;
	for (int i = 0; i < nb_pixels; i++) res += C_getSaturation (img[i]);
	res /= nb_pixels;
}

void I_addWhite (Color *img, int nb_pixels, float coef) {
	for (int i = 0; i < nb_pixels; i++) img[i] = C_addWhite(img[i], coef);
}

void I_addBlack (Color *img, int nb_pixels, float coef) {
	for (int i = 0; i < nb_pixels; i++) img[i] = C_addBlack(img[i], coef);
}

void I_addTint (Color *img, int nb_pixels, int col, float coef) {
	for (int i = 0; i < nb_pixels; i++) img[i] = C_addTint(img[i], col, coef);
}

void I_addHighlights (Color *img, int nb_pixels, float coef) {
	for (int i = 0; i < nb_pixels; i++) img[i] = C_addHighlights(img[i], coef);
}
