...
int main () {
	...
	// printf("%lf\n", I_getSaturation(img, nbpixels));
	I_addHighlights(img, nbpixels, -0.08);
	I_saturation(img, nbpixels, 0.5);
	// I_coef (temp, nbpixels, 0.8);
	I_addWhite(img, nbpixels, 0.05);
	I_addBlack(img, nbpixels, 0.1);
	I_addTint(img, nbpixels, BLUE, 0.1);
	I_addTint(img, nbpixels, GREEN, 0.05);
	PPM_save(p, img, "sat6.ppm");
	// printf("%lf\n", I_getSaturation(img, nbpixels));
