...
struct ppm PPM_new(char *ppmfilepath)
{
	...
	else
	{
		...
		else
		{
			...
			ppm_new.pixels = (Color*)calloc(ppm_new.nbpixels,sizeof(Color));
...
void PPM_save(struct ppm p, Color *image, char *outputfile)
{
	FILE *ppmfile;
	int n;
	...
	{
		fprintf(ppmfile,"P3\n");
		fprintf(ppmfile,"# File created by ppmfilecreator Kamal Aghayev\n");
		fprintf(ppmfile,"%d %d\n",p.width,p.height);
		fprintf(ppmfile,"%d\n",255);
		for(n=0;n<p.nbpixels;n++)
		{
